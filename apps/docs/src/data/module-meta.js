...
const additionalExamples = {
	...
	table: [
		{
			title: "Loading rows",
			description:
				"Skeleton rows stand in for data while loading; the region is marked busy and announced through loadingLabel.",
		},
	],
	separator: [
		{
	...
	avatar: [
		{
			title: "Fallbacks",
			description:
				"Initials replace a missing or failed image; a generic icon covers unnamed users.",
		},
	],
	"button-group": [
		{
	...
	"radio-group": [
		{
			...
			description: "Use horizontal orientation for two or three short options.",
		},
	],
	slider: [
		{
	...
	combobox: [
		{
			...
			description:
				"Options can be disabled with an explanatory empty message for no matches.",
		},
	],
	chip: [
		{
	...
	drawer: [
		{
			title: "Left edge",
			description:
				'Use side="left" when the panel continues a left-side navigation context.',
		},
	],
	"hover-card": [
		{
	...
	"description-list": [
		{
			title: "Two-column details",
			description: "Use the grid layout for wider detail panels.",
		},
	],
	toggle: [
		{
	...
	"multi-select": [
		{
			title: "Disabled options",
			description:
				"Individual options can be disabled while the rest stay selectable.",
		},
	],
	"date-picker": [
		{
			title: "Bounded dates",
			description: "Use minDate and maxDate for booking-style windows.",
		},
	],
	"number-input": [
		{
			title: "Custom steps",
			description:
				"Set step for increments that match the domain, like 0.5 or 10.",
		},
	],
	"password-input": [
		{
			...
			description:
				'Pair with autocomplete="current-password" for sign-in forms.',
		},
	],
	"file-upload": [
		{
			...
			description:
				"Constrain with accept when only certain file types are valid.",
		},
	],
	"data-table": [
		{
	...
	"pin-input": [
		{
			title: "Masked codes",
			description:
				"masked renders password-style cells for codes that should not linger on screen.",
		},
	],
	"tags-input": [
		{
			title: "Capped lists",
			description:
				"Set max when the domain allows only a fixed number of tags.",
		},
	],
	"input-group": [
		{
			...
			description:
				"A trailing addon works for units like GB or currency codes.",
		},
	],
	editable: [
		{
			title: "Empty values",
			description:
				"The placeholder keeps an empty value discoverable and clickable.",
		},
	],
	"time-picker": [
		{
			title: "12-hour cycle",
			description:
				"hourCycle={12} swaps the 0–23 hour field for 1–12 with an AM/PM toggle.",
		},
	],
	"date-range-picker": [
		{
	...
	popconfirm: [
		{
			title: "Default tone",
			description:
				"Use the default tone for non-destructive confirmations such as publishing.",
		},
	],
	"context-menu": [
		{
	...
	stat: [
		{
			...
			description:
				"Pass a Sparkline as children when the metric benefits from a trend shape.",
		},
	],
	list: [
		{
	...
	sparkline: [
		{
			title: "Bar variant",
			description:
				'Use variant="bar" for discrete periods such as daily totals.',
		},
	],
	calendar: [
		{
			title: "Bounded dates",
			description:
				"Use min and max for booking-style windows like Calendar inside pickers.",
		},
	],
	result: [
		{
	...
	grid: [
		{
			title: "Fixed columns",
			description:
				"Use columns when the layout must hold an exact track count.",
		},
	],
	resizable: [
		{
	...
	carousel: [
		{
			title: "Looping",
			description:
				"loop keeps prev and next enabled by wrapping past the ends.",
		},
	],
	"side-rail": [
		{
...
const guidanceById = {
	...
	table: {
		useWhen: "Rows and columns are the clearest way to compare records.",
		avoidWhen: "The content is a single object or needs a narrative layout.",
		behavior: "Columns own rendering and rows require stable keys.",
		responsive:
			"Keep the table readable with horizontal scrolling or a deliberate compact projection.",
	},
	separator: {
		useWhen: "Related groups need a clear visual or semantic boundary.",
		avoidWhen: "Spacing alone communicates hierarchy.",
		behavior:
			"Use decorative mode when the divider carries no document meaning.",
		responsive: "Prefer horizontal separators in stacked mobile layouts.",
	},
	avatar: {
		useWhen: "A person or entity needs a compact visual identity.",
		avoidWhen:
			"The image carries information beyond identity and needs a caption.",
		behavior:
			"Falls back from image to initials to a generic icon; alt text defaults to the name.",
		responsive:
			"Pick one size per context and keep it fixed across breakpoints.",
	},
	"button-group": {
	...
	stat: {
		useWhen: "A headline metric needs a label and trend context.",
		avoidWhen: "Several metrics compare in rows; use Table.",
		behavior:
			"Delta direction picks the icon and tone; an sr-only prefix states the direction.",
		responsive:
			"Value and delta wrap together; let stats stack in narrow grids.",
	},
	list: {
		useWhen:
			"Items share one row structure with optional leading and trailing content.",
		avoidWhen: "Records need column alignment; use Table.",
		behavior: "onClick rows become buttons; dense reduces padding via context.",
		responsive:
			"Text truncates rather than wrapping; trailing content stays pinned.",
	},
	sparkline: {
		useWhen: "A trend shape adds context next to a metric.",
		avoidWhen: "Precise values matter; pair with Stat or use a full chart.",
		behavior:
			'role="img" with an aria-label plus a hidden min, max, and last summary.',
		responsive: "Set explicit width and height per context.",
	},
	calendar: {
		useWhen: "The user picks one date directly from a month grid.",
		avoidWhen: "A date typed in a field is enough, or a range is needed.",
		behavior:
			"min, max, and disabledDates constrain days; selected dates keep readable contrast and the month label is announced.",
		responsive: "The grid keeps a fixed comfortable cell size.",
	},
	result: {
		useWhen:
			"A route or panel resolves to a single outcome such as success, error, or 404.",
		avoidWhen: "The message is local to a field or task; use Alert or Field.",
		behavior:
			"The icon is decorative; title and description carry the meaning with optional actions.",
		responsive: "Content centers and wraps within a readable measure.",
	},
	stack: {
		useWhen: "Children flow along one axis with even spacing.",
		avoidWhen: "The layout needs two-dimensional tracks; use Grid.",
		behavior:
			"Numeric gaps follow the spacing scale; alignment maps to flexbox values.",
		responsive: "Combine wrap with row direction for adaptive toolbars.",
	},
	grid: {
		useWhen: "Children distribute across columns, fixed or responsive.",
		avoidWhen: "Items flow in one line; use Stack.",
		behavior: "minChildWidth enables auto-fit tracks; columns fixes the count.",
		responsive: "Auto-fit collapses tracks as the container narrows.",
	},
	resizable: {
		useWhen: "Users adjust the split between two panes.",
		avoidWhen: "The layout is fixed; do not add interaction without a need.",
		behavior:
			"Handles drag, step with arrow keys, and reset on double-click; sizes are percentages.",
		responsive: "Set min sizes so panes stay usable on narrow screens.",
	},
	"aspect-ratio": {
		useWhen: "Media must hold a consistent shape across widths.",
		avoidWhen: "Content should size naturally.",
		behavior:
			"The wrapper reserves the ratio and clips overflow with rounded corners.",
		responsive: "The box scales with its container while holding the ratio.",
	},
	"visually-hidden": {
...
export const moduleGuidance = Object.fromEntries(
	modules.map((module) => [
		module.id,
		guidanceById[module.id] ?? {
			useWhen: `When ${module.name} is the clearest fit for the interaction.`,
			avoidWhen:
				"When a simpler native element communicates the intent better.",
			behavior: "Preserve the component contract and native keyboard behavior.",
			responsive:
				"Let the consumer control layout while retaining the component semantics.",
		},
	]),
);
