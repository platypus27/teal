...
const guidanceById = {
  ...
  'scroll-area': { useWhen: 'A panel needs a bounded height with theme-consistent scrollbars.', avoidWhen: 'The page itself should scroll; do not nest page-level scrolling.', behavior: 'The custom thumb appears over a transparent track only where scrolling is possible; the viewport is keyboard focusable.', responsive: 'Set maxHeight in relative units so the region adapts to viewport height.' },
  ...
  'multi-select': { useWhen: 'Users pick several values from a filterable list.', avoidWhen: 'Only one value is allowed; use Select or Combobox.', behavior: 'Options toggle without closing and pills remove through compact 24px touch targets.', responsive: 'Pills wrap inside the control as values accumulate.' },
  ...
  'number-input': { useWhen: 'A numeric value benefits from quick stepping.', avoidWhen: 'The value is an identifier, not a quantity; use Input.', behavior: 'Steppers use 24px touch targets and blur clamps to min/max; empty means undefined.', responsive: 'The field fills its container; constrain it in the form layout.' },
  ...
  'code-block': { useWhen: 'Code or commands should be readable and copyable.', avoidWhen: 'A single identifier in prose; use inline code styling.', behavior: 'The copy action confirms with an icon swap and announces via its label; long code regions are keyboard focusable.', responsive: 'Long lines scroll horizontally instead of wrapping.' },
  ...
  'input-group': { useWhen: 'An input needs a fixed prefix or suffix such as a protocol or unit.', avoidWhen: 'The accessory is interactive; use separate controls instead.', behavior: 'The group owns the border and focus ring, so the whole box highlights as one control.', responsive: 'Addons stay fixed width while the input flexes.' },
  ...
  calendar: { useWhen: 'The user picks one date directly from a month grid.', avoidWhen: 'A date typed in a field is enough, or a range is needed.', behavior: 'min, max, and disabledDates constrain days; selected dates keep readable contrast and the month label is announced.', responsive: 'The grid keeps a fixed comfortable cell size.' },
  ...
  carousel: { useWhen: 'Peer items page through a bounded region.', avoidWhen: 'All content should be visible at once; use Grid.', behavior: 'Scroll-snap track with buttons, compact dots with 24px touch targets, and arrow keys; slides announce their position.', responsive: 'Slides take full track width; keep content readable at mobile widths.' }
}
