...
export function Button({ children, variant = 'primary', className = '', as: Component = 'button', ...props }) {
  ...
  return (
    <Component className={cn(base, styles[variant], className)} {...props}>
      {children}
    </Component>
  )
}
...
export function IconButton({ children, className = '', title, ...props }) {
  return (
    <button
      type="button"
      title={title}
      className={cn('flex h-9 w-9 items-center justify-center rounded-full text-on-surface-variant transition hover:bg-surface-container-high disabled:opacity-60', className)}
      {...props}
    >
      {children}
    </button>
