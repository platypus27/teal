...
export const Calendar = forwardRef<HTMLDivElement, CalendarProps>(function Calendar(
  { className, disabledDates, max, min, onMonthChange, onSelect, value, visibleMonth, ...props },
  ref,
) {
  const [internalMonth, setInternalMonth] = useState<Date>(() => addMonths(value ?? new Date(), 0))
  ...
  return (
    <div ref={ref} className={cn('teal-u-w-fit teal-u-p-3', className)} {...props}>
      <div className="teal-u-flex teal-u-items-center teal-u-justify-between teal-u-pb-2">
        <IconButton label="Previous month" size="sm" onClick={() => setMonth(addMonths(viewMonth, -1))}>
        ...
        <IconButton label="Next month" size="sm" onClick={() => setMonth(addMonths(viewMonth, 1))}>
