...
const CarouselSlideImpl = forwardRef<HTMLDivElement, InternalSlideProps>(
	function CarouselSlide({ _index, _total, className, ...props }, ref) {
		return (
			<div
				ref={ref}
				role="group"
				...
				aria-label={
					_index !== undefined && _total !== undefined
						? `${_index + 1} of ${_total}`
						: undefined
				}
				className={cn(
					"teal-u-w-full teal-u-shrink-0 teal-u-snap-start",
					className,
				)}
				{...props}
			/>
...
export const CarouselSlide =
	CarouselSlideImpl as React.ForwardRefExoticComponent<
							...
							>
								<span
									...
									className={cn(
										...
										index === active
											? "teal-u-bg-primary"
											: "teal-u-bg-outline-variant group-hover:teal-u-bg-outline",
									)}
								/>
							</button>
						))}
