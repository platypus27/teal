...
function formatRange(from: Date, to: Date) {
  const start = from.getFullYear() === to.getFullYear() ? dayFormatter.format(from) : dayWithYearFormatter.format(from)
...
export const DateRangePicker = forwardRef<HTMLInputElement, DateRangePickerProps>(function DateRangePicker(
  ...
  const [focusedDate, setFocusedDate] = useState<Date>(() => selected.from ?? new Date())
  ...
  const presets = [
    ...
    { label: 'Last 7 days', from: addDays(startOfDay(today), -6), to: startOfDay(today) },
    { label: 'Last 30 days', from: addDays(startOfDay(today), -29), to: startOfDay(today) },
  ]
            ...
            <div ref={gridRef} onKeyDown={handleGridKeyDown}>
              ...
              <div className="teal-u-grid teal-u-grid-cols-7 teal-u-justify-items-center">
                ...
                {days.map((day) => {
                  ...
                  const isInRange =
                    from !== null && to !== null && day.getTime() > from.getTime() && day.getTime() < to.getTime()
