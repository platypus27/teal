...
describe('InputGroup', () => {
  ...
  it('renders addons borderless with a hairline separator on the inner side', () => {
    ...
    const leading = screen.getByText('$')
    const trailing = screen.getByText('.00')
    const classList = (element: HTMLElement) => element.className.split(' ')
    expect(leading.className).toContain('teal-u-border-e')
    expect(classList(leading)).not.toContain('teal-u-border-s')
    expect(trailing.className).toContain('teal-u-border-s')
    expect(classList(trailing)).not.toContain('teal-u-border-e')
