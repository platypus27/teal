...
describe("Calendar", () => {
	it("renders the month heading and a full six-week grid", () => {
		render(
			<Calendar value={null} onSelect={vi.fn()} visibleMonth={june2025} />,
		);
	...
	it('marks today with aria-current="date"', () => {
		...
		expect(
			candidates.find((day) => day.getAttribute("aria-current") === "date"),
		).toBeDefined();
	});
	...
	it("dims days outside the visible month", () => {
		...
		const julyFirst = screen
			...
			.find((day) =>
				day.className.includes("teal-u-text-on-surface-variant/50"),
			);
	...
	it("calls onSelect with the clicked date", async () => {
		...
		render(
			<Calendar value={null} onSelect={onSelect} visibleMonth={june2025} />,
		);
	...
	it("changes the visible month internally when uncontrolled", async () => {
		...
		const julyLabel = new Intl.DateTimeFormat(undefined, {
			month: "long",
			year: "numeric",
		}).format(new Date(2025, 6, 1));
