...
describe('NumberInput', () => {
  it('renders the label and initial value', () => {
    ...
    expect(screen.getByRole('spinbutton', { name: 'Team size' })).toHaveValue(3)
  })
  ...
  it('clamps stepper values at min and max', async () => {
    const user = userEvent.setup()
    const onValueChange = vi.fn()
    render(<NumberInput label="Team size" defaultValue={5} min={1} max={6} onValueChange={onValueChange} />)
    const input = screen.getByRole('spinbutton', { name: 'Team size' })
  ...
  it('applies the step and still clamps to the bounds', async () => {
    const user = userEvent.setup()
    const onValueChange = vi.fn()
    render(<NumberInput label="Velocity" defaultValue={2} min={0} max={9} step={5} onValueChange={onValueChange} />)
  ...
  it('starts stepping from min when the field is empty', async () => {
    const user = userEvent.setup()
    const onValueChange = vi.fn()
    render(<NumberInput label="Team size" min={2} onValueChange={onValueChange} />)
