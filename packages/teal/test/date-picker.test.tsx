...
describe('DatePicker', () => {
  ...
  it('opens the calendar on click showing the selected month', async () => {
    ...
    render(<DatePicker label="Due date" defaultValue={new Date(2024, 0, 15)} />)
    const input = screen.getByRole('textbox', { name: 'Due date' })
  ...
  it('navigates between months with the header buttons', async () => {
    ...
    expect(screen.getByText(monthLabel(new Date(2024, 1, 1)))).toBeInTheDocument()

    await user.click(screen.getByRole('button', { name: 'Previous month' }))
    await user.click(screen.getByRole('button', { name: 'Previous month' }))
    expect(screen.getByText(monthLabel(new Date(2023, 11, 1)))).toBeInTheDocument()
  })

  it('selects a day, reports it, and closes the calendar', async () => {
    const user = userEvent.setup()
    const onValueChange = vi.fn()
    render(<DatePicker label="Due date" defaultValue={new Date(2024, 0, 10)} onValueChange={onValueChange} />)
    const input = screen.getByRole('textbox', { name: 'Due date' })
    ...
    expect(screen.queryByText(monthLabel(new Date(2024, 0, 1)))).not.toBeInTheDocument()
  })

  it('supports arrow-key navigation and Enter selection', async () => {
    const user = userEvent.setup()
    const onValueChange = vi.fn()
    render(<DatePicker label="Due date" defaultValue={new Date(2024, 0, 15)} onValueChange={onValueChange} />)
    const input = screen.getByRole('textbox', { name: 'Due date' })
