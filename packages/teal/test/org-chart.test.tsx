...
describe('OrgChart', () => {
  ...
  it('bridges sibling gaps so connectors stay centered on nodes of differing widths', () => {
    ...
    expect(horizontal[0]!.className).toContain('teal-u-start-1/2')
    expect(horizontal[0]!.className).toContain('-teal-u-end-2')
    // Last child: line runs from past its left edge to the node center.
    expect(horizontal[1]!.className).toContain('-teal-u-start-2')
    expect(horizontal[1]!.className).toContain('teal-u-end-1/2')
  })
})
